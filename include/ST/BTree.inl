...
bool BTree<T, N>::m_debugMode {false};

template <typename T, int N>
uint32 BTree<T, N>::m_filesCount {0};
...
template <typename T, int N>
BTree<T, N>::BTree(const std::vector<AttachedPair<T>>& data)
    : m_root      {nullptr},
      m_lockPhase {true},
      m_nextIndex {sizeof(std::size_t)}
{
...
template <typename T, int N>
BTree<T, N>::BTree(const std::initializer_list<AttachedPair<T>>& l)
    : m_root      {nullptr},
      m_lockPhase {true},
      m_nextIndex {sizeof(std::size_t)}
{
...
template <typename T, int N>
void BTree<T, N>::makeEmpty()
{
    makeEmpty(m_root);
}

template <typename T, int N>
void BTree<T, N>::makeEmpty(Node* root)
{
    if(root == nullptr)
    {
        return;
...
template <typename T, int N>
BTreeNode<T, N>* BTree<T, N>::find(const AttachedPair<T>& value, int& index)
{
    return find(value, m_root, index);
}

template <typename T, int N>
BTreeNode<T, N>* BTree<T, N>::find(const AttachedPair<T>& value, Node*& root, int& index)
{
    if(root == nullptr)
    {
        return nullptr;
...
template <typename T, int N>
void BTree<T, N>::insertKey(const AttachedPair<T>& value)
{
...
template <typename T, int N>
void BTree<T, N>::insertKey(const AttachedPair<T>& value, Node*& root)
{
    if(root == nullptr)
    {
...
template <typename T, int N>
void BTree<T, N>::updateKey(const Node* node)
{
    if(node != nullptr)
    {
        setTransactionState(1);
...
template <typename T, int N>
void BTree<T, N>::deleteKey(const AttachedPair<T>& value)
{
    setTransactionState(1);
        m_writeTarget = &m_file;
...
template <typename T, int N>
void BTree<T, N>::deleteKey(const AttachedPair<T>& value, Node*& root)
{
    //
}

template <typename T, int N>
void BTree<T, N>::loadAll()
{
    loadAll(m_root);
}

template <typename T, int N>
void BTree<T, N>::loadAll(Node*& root)
{
    if(root == nullptr)
    {
        return;
...
template <typename T, int N>
void BTree<T, N>::exportToFile(const std::string& title, bool isFinal)
{
    if(m_root == nullptr)
    {
        return;
...
template <typename T, int N>
void BTree<T, N>::labelNode(Node* node)
{
    if(node != nullptr)
    {
        std::stringstream ss;
...
template <typename T, int N>
void BTree<T, N>::splitChild(Node* node, const std::size_t index)
{
...
template <typename T, int N>
void BTree<T, N>::insertKeyNonFull(const AttachedPair<T>& value, Node*& root)
{
    int i = root->size - 1;
...
template <typename T, int N>
void BTree<T, N>::createFileIfNotExists()
{
    m_file.open("data/data.bt", std::ios::in | std::ios::out | std::ios::binary);
    m_bkFile.open("data/dataBK.bt", std::ios::in | std::ios::out | std::ios::binary);
...
template <typename T, int N>
void BTree<T, N>::diskWrite(const Node* node)
{
...
template <typename T, int N>
void BTree<T, N>::diskRead(Node*& node, const std::size_t index)
{
...
template <typename T, int N>
std::size_t BTree<T, N>::getRootIndex()
{
...
template <typename T, int N>
void BTree<T, N>::setRootIndex(const std::size_t index)
{
...
template <typename T, int N>
void BTree<T, N>::setTransactionState(const char state)
{
